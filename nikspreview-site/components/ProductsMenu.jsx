...
export default function ProductsMenu({ root, selectedLeaf, onLeafSelect, theme }) {
    ...
    return (
      <div style={{ marginLeft: depth * 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
          ...
          {isParent && (
            <span onClick={() => toggle(node.id)} style={{ cursor: "pointer", fontSize: 14 }}>
              {isExpanded ? "▾" : "▸"}
            </span>
          )}
        </div>
  ...
  return (
    <nav style={{ padding: 12, background: theme.background, border: `1px solid ${theme.accent}`, borderRadius: 12 }}>
