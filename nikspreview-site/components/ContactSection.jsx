...
export default function ContactSection({
  ...
  useEffect(() => {
    if (!location) {
      console.log("ContactSection: No location provided");
      return setAreaCity("");
    }

    console.log("ContactSection: Location data:", location);
    ...
    if (location.lat && location.lng) {
      const fetchAreaCity = async () => {
        try {
          console.log(`Fetching reverse geocode for: ${location.lat}, ${location.lng}`);
          ...
          console.log("Setting areaCity:", areaCity);
          setAreaCity(areaCity);
        } catch (err) {
      ...
      console.log("ContactSection: Location missing lat/lng", location);
    }
  }, [location]);
  ...
  return (
    ...
    >
      <div style={{ textAlign: "center", marginBottom: "40px" }}>
        <h2 style={{ fontSize: "32px", fontWeight: "bold", marginBottom: "10px", color: theme.primary || fallbackTheme.primary }}>
          Contact Us
        </h2>
        <p style={{ fontSize: "16px", color: theme.text || fallbackTheme.text }}>Get in touch with us</p>
      </div>
      ...
      >
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          ...
          <div
            style={{
              ...
              borderRadius: "10px",
              background: theme.cardBg || fallbackTheme.cardBg,
              boxShadow: "0 2px 6px rgba(0,0,0,0.1)",
            }}
          >
            <h4 style={{ marginBottom: "10px", fontWeight: "bold", color: theme.primary || fallbackTheme.primary }}>Phone</h4>
            <p>{contactNumber || "-"}</p>
          </div>
          ...
          >
            <h4 style={{ marginBottom: "10px", fontWeight: "bold", color: theme.primary || fallbackTheme.primary }}>Location</h4>
            {location && location.lat && location.lng ? (
              <>
                {areaCity && <p style={{ marginBottom: "10px", color: theme.text || fallbackTheme.text }}>{areaCity}</p>}
                <iframe
                  title="map"
                  width="100%"
                  height="250"
                  style={{ border: 0, borderRadius: "8px" }}
                  src={`https://www.openstreetmap.org/export/embed.html?bbox=${location.lng - 0.01}%2C${location.lat - 0.01}%2C${
                    location.lng + 0.01
                  }%2C${location.lat + 0.01}&layer=mapnik&marker=${location.lat}%2C${location.lng}`}
                />
              </>
            ) : (
              <p style={{ color: "#9CA3AF", fontSize: "14px" }}>Location not available for this vendor.</p>
            )}
          </div>
          ...
          >
            <h4 style={{ marginBottom: "10px", fontWeight: "bold", color: theme.primary || fallbackTheme.primary }}>Business Hours</h4>
              ...
              {businessHours.map((item, idx) => (
                <li
                  ...
                  style={{
                    ...
                    color: todayIndex === idx ? theme.accent || fallbackTheme.accent : theme.text || fallbackTheme.text,
                  }}
                >
                  <span>{item.day}</span>
        ...
        >
          <input
            ...
            required
            style={{ padding: "12px", border: "1px solid #ccc", borderRadius: "8px", backgroundColor: theme.inputBg || fallbackTheme.inputBg }}
          />
          <input
            type="tel"
          ...
          <textarea
            ...
            rows="4"
            style={{ padding: "12px", border: "1px solid #ccc", borderRadius: "8px", resize: "none", backgroundColor: theme.inputBg || fallbackTheme.inputBg }}
          />
          <button
            type="submit"
            style={{
              ...
              color: theme.buttonText || fallbackTheme.buttonText,
              border: "none",
              borderRadius: "30px",
              fontWeight: "bold",
              cursor: "pointer",
