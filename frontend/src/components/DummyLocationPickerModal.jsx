...
export default function DummyLocationPickerModal({
  show,
  onClose,
  vendorId,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // fetch existing to preserve nearbyLocations
      let nearbyLocations = [];
      try {
        const cur = await axios.get(`${API_BASE_URL}/api/dummy-vendors/${vendorId}/location`);
        nearbyLocations = cur?.data?.location?.nearbyLocations || [];
      } catch {}
      const res = await axios.put(
        `${API_BASE_URL}/api/dummy-vendors/${vendorId}/location`,
        { lat, lng, areaCity, address: areaCity || "", nearbyLocations }
      );
      onLocationSave?.(res.data.location);
      onClose?.();
    } catch (err) {
      console.error("Failed to save dummy location", err);
      alert("Failed to save dummy location.");
    } finally {
      setIsSaving(false);
    }
  };
