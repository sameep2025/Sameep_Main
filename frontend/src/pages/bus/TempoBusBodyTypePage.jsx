...
function BodyTypeCard({ item, onEdit, onDelete }) {
  ...
  return (
    ...
    >
      ...
      {imageSrc ? (
        <img src={imageSrc} alt={item.name} style={{ width: 100, height: 100, objectFit: "contain", borderRadius: 12 }} />
      ) : (
        <div style={{ width: 100, height: 100, borderRadius: 12, background: "#ccc", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 36, color: "#fff" }}>?</div>
      )}
...
export default function TempoBusBodyTypePage() {
  ...
  return (
    <div style={{ padding: 30 }}>
      <h2 style={{ color: "#00AEEF" }}>Tempo Mini Bus Body Types</h2>
      <button onClick={() => { setShowModal(true); setEditItem(null); setForm({ name: "", file: null, preview: null }); }} style={{ marginBottom: 20, background: "#00AEEF", color: "#fff", border: "none", borderRadius: 8, padding: "10px 18px", cursor: "pointer" }}>+ Add Body Type</button>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(250px, 1fr))", gap: 16 }}>
        {items.map((i) => <BodyTypeCard key={i._id} item={i} onEdit={handleEdit} onDelete={handleDelete} />)}
      </div>
      ...
      {showModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 1000 }}>
          <div style={{ background: "#fff", padding: 30, borderRadius: 12, width: 400 }}>
            <h3>{editItem ? "Edit Body Type" : "Add Body Type"}</h3>
