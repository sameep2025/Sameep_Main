...
// Card for each display type
function DisplayTypeCard({ displayType, onEdit, onDelete }) {
  return (
    <div
      style={{
...
// Modal for add/edit
function DisplayTypeModal({ show, onClose, onSave, initialData }) {
  const [name, setName] = useState("");

  // Reset name whenever modal opens or initialData changes
  useEffect(() => {
    if (show) {
    ...
    >
      ...
      >
        <h3 style={{ marginTop: 0, color: "#00AEEF" }}>
          {initialData ? "Edit Display Type" : "Add Display Type"}
        </h3>
        <form
          onSubmit={submit}
...
// Main Display Type Page
export default function DisplayTypePage() {
  ...
  const fetchDisplayTypes = async () => {
    try {
      ...
      console.error(err);
      alert("Failed to fetch display types");
    }
  };
  ...
  const handleDelete = async (displayType) => {
  ...
  &gt; Display Types
      ...
      <h2 style={{ color: "#00AEEF" }}>Display Types</h2>
      <button
        onClick={() => {
          setEditing(null);
          setShowModal(true);
