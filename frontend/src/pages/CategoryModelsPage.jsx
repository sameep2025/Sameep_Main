...
// Card for each category model
// Card for each category model
function CategoryModelCard({ item, onEdit, onDelete }) {
  return (
    <div
      style={{
...
// Modal
function CategoryModelModal({ show, onClose, onSave, initialData }) {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <h3 style={{ marginTop: 0, color: "#00AEEF" }}>
          {initialData ? "Edit Category Model" : "Add Category Model"}
        </h3>
...
export default function CategoryModelsPage() {
  ...
  const fetchItems = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/masters", { params: { type: "categoryModel" } });
      setItems(res.data);
    } catch (err) {
      console.error(err);
      alert("Failed to fetch category models");
    }
  };
  ...
  const handleSave = async (data) => {
    try {
      if (data._id) {
        ...
        await axios.post("http://localhost:5000/api/masters", { name: data.name, type: "categoryModel" });
      }
      fetchItems();
      setShowModal(false);
      setEditing(null);
  ...
  &gt; Category Models
      ...
      <h2 style={{ color: "#00AEEF" }}>Category Models</h2>
