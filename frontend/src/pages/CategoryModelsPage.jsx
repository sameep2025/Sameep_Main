...
export default function CategoryModelsPage() {
  ...
  const fetchItems = async () => {
    try {
      const res = await axios.get(`${API_BASE_URL}/api/masters`, { params: { type: "categoryModel" } });
      setItems(res.data);
    } catch (err) {
      console.error(err);
  ...
  const handleSave = async (data) => {
    try {
      if (data._id) {
        ...
        await axios.post(`${API_BASE_URL}/api/masters`, { name: data.name, type: "categoryModel" });
      }
      fetchItems();
      setShowModal(false);
      setEditing(null);
